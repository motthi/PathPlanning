...
class IdealRobot:
    ...
    def drawTakenPath(self, poses, ax, elems=None, color="black", linewidth=0.5):
        if elems is None:
            ax.plot([e[0] for e in self.poses], [e[1] for e in poses], linewidth=linewidth, color=color)
        else:
            elems += ax.plot([e[0] for e in self.poses], [e[1] for e in poses], linewidth=linewidth, color=color)
...
if __name__ == "__main__":
    ...
    robot2 = Robot(np.array([0, 0, 0]), circling1, IdealCamera(m))
    world.append(robot1)
    world.append(robot2)
