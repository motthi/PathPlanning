...
class World:
    def __init__(self, time_span, time_interval, map_size=np.array([10, 10]).T, debug=False):
        self.objects = []
        self.debug = debug
...
class Obstacle:
    def __init__(self, x, y):
        self.pos = np.array([x, y]).T.astyp
        self.id = None

    def draw(self, ax, elems):
        c = ax.scatter(self.pos[0], self.pos[1], s=10,
                       marker="o", label="landmarks", color="black")
        elems.append(c)
